...
public final class OnlyZombiesSpawnLogic {
    ...
    public static Entity createReplacementIfNeeded(ServerLevel level, Entity entity) {
        ...
        if (!shouldReplace(level, entity, spawnReason)) {
            return entity;
        }
    ...
    private static boolean shouldReplace(ServerLevel level, Entity entity, EntitySpawnReason spawnReason) {
        if (!(entity instanceof Mob)) {
            return false;
        }

        if (SpawnWhitelistConfig.isDimensionDisabled(level)) {
            return false;
        }
        ...
        if (entity.getType() == EntityType.ZOMBIE || SpawnWhitelistConfig.isMobAllowed(entity.getType()) || isExcludedBoss(entity)) {
            return false;
        }
